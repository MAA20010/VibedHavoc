...
var CobraServer = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Force reset debug flags to ensure no persistence between runs
		if !cmd.Flags().Changed("debug-dev") {
			flags.Server.DebugDev = false
		}
		if !cmd.Flags().Changed("debug") {
			flags.Server.Debug = false
		}
		if !cmd.Flags().Changed("send-logs") {
			flags.Server.SendLogs = false
		}
