...
// AgentLoadFromDB loads an agent from the database, adds it to memory, and notifies the UI.
// Used when an agent beacons in but isn't in the in-memory list (e.g. after server restart).
func (t *Teamserver) AgentLoadFromDB(AgentID int) *agent.Agent {
	loaded := t.DB.AgentGetByID(AgentID)
	if loaded == nil {
		return nil
	}

	// Add to in-memory list (DB already has it, AgentAdd's DB insert will no-op)
	t.AgentAdd(loaded)

	// Notify UI so the agent appears in the client
	t.AgentSendNotify(loaded)

	logger.Info(fmt.Sprintf("Agent %s restored from DB into memory", loaded.NameID))
	return loaded
}
