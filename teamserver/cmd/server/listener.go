	...
	case handlers.LISTENER_HTTP:
		...
		for i := range t.Listeners {
			...
			if t.Listeners[i].Name == Config.(handlers.HTTPConfig).Name {
				cfg := Config.(handlers.HTTPConfig)
				listener := t.Listeners[i].Config.(*handlers.HTTP)

				// Callback metadata -- affects new agent builds, safe to update at runtime
				listener.Config.Hosts = cfg.Hosts
				listener.Config.HostRotation = cfg.HostRotation
				listener.Config.HostHeader = cfg.HostHeader
				listener.Config.PortConn = cfg.PortConn

				// Request shaping -- safe to update at runtime
				listener.Config.UserAgent = cfg.UserAgent
				listener.Config.Headers = cfg.Headers
				listener.Config.Uris = cfg.Uris
				listener.Config.Proxy = cfg.Proxy
				listener.Config.BehindRedir = t.Profile.Config.Demon.TrustXForwardedFor

				// NOTE: HostBind and PortBind are NOT updated here.
				// Changing them requires stopping and restarting the HTTP server
				// on the new address, which is not supported via live edit.
			}
...
// GetSMBListenerPSKs returns all PSKs from SMB pivot listeners.
// Used for SMB pivot KEX when the child's specific listener is unknown.
func (t *Teamserver) GetSMBListenerPSKs() [][]byte {
	var psks [][]byte
	for _, listener := range t.Listeners {
		if listener.Type == handlers.LISTENER_PIVOT_SMB {
			if smbHandler, ok := listener.Config.(*handlers.SMB); ok {
				if len(smbHandler.Config.PSK) >= 32 {
					psks = append(psks, []byte(smbHandler.Config.PSK))
				}
			}
		}
	}
