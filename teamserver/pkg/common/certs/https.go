...
func generateCertificate(caType string, subject pkix.Name, isCA bool, isClient bool, privateKey interface{}) ([]byte, []byte) {
    ...
    // Sign certificate — self-signed in all cases (parent = template)
    var certErr error
    var derBytes []byte
...
func HTTPSGenerateRSACertificate(host string) ([]byte, []byte, error) {
    ...
    // isCA=false: listener certs should NOT be CA certs.
    // A CA cert on an HTTPS listener is anomalous and detectable by network forensics.
