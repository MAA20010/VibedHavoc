...
func generateRandomPSK(numBytes int) string {
	b := make([]byte, numBytes)
	if _, err := rand.Read(b); err != nil {
		return common.RandomString(numBytes * 2)
	}
	return hex.EncodeToString(b)
}

// sanitizeServiceName removes dangerous characters that could be used for command injection
// Only allows alphanumeric characters, spaces, hyphens, and underscores
func sanitizeServiceName(name string) string {
	// Use regex to match only safe characters
	reg := regexp.MustCompile(`[^a-zA-Z0-9\s\-_]`)
	sanitized := reg.ReplaceAllString(name, "")

	// Trim whitespace and limit length to prevent excessively long names
	sanitized = strings.TrimSpace(sanitized)
	if len(sanitized) > 64 {
		sanitized = sanitized[:64]
	}

	// Ensure we have at least something, fallback to random string if empty
	if len(sanitized) == 0 {
		sanitized = common.RandomString(8)
	}

	return sanitized
}

// validateDefineValue checks if a compiler define value contains dangerous characters
func validateDefineValue(value string) bool {
	// Check for shell metacharacters that could be used for command injection
	// Only block the most dangerous patterns that are unlikely to appear in legitimate compiler defines
	dangerousPatterns := []string{
		"; ", ";", // Command separator (with and without space)
		"&& ", "||", // Logical operators
...
func (b *Builder) Build() bool {
	...
	if b.compilerOptions.Config.DebugDev {
		...
		b.compilerOptions.Defines = append(b.compilerOptions.Defines, "HASH_DEBUG")
	} else {
		b.compilerOptions.Defines = append(b.compilerOptions.Defines, "NDEBUG")
	...
	for _, define := range b.compilerOptions.Defines {
		// Validate define to prevent command injection
		if !validateDefineValue(define) {
			logger.Error("Dangerous characters detected in compiler define: " + define)
			if !b.silent {
				b.SendConsoleMessage("Error", "Dangerous characters detected in compiler define. Build aborted for security.")
			}
			return false
		}
	...
	case FILETYPE_WINDOWS_RAW_BINARY:
		...
		logger.Info("Creating DLL sub-builder with same config")
	...
	// Compile version resource for proper PE metadata (Task Manager title)
	var ResObj string
	if b.FileType == FILETYPE_WINDOWS_EXE || b.FileType == FILETYPE_WINDOWS_SERVICE_EXE {
		ResObj = b.CompileDir + utils.GenerateID(10) + ".res.o"
		var ResCompile string

		// Determine windres executable based on architecture
		var windresExe string
		if b.config.Arch == ARCHITECTURE_X64 {
			windresExe = "x86_64-w64-mingw32-windres"
		} else {
			windresExe = "i686-w64-mingw32-windres"
		}

		// Create windres command to compile version resource
		ResCompile = fmt.Sprintf("%s src/main/version.rc -o %s", windresExe, ResObj)

		if !b.silent {
			b.SendConsoleMessage("Info", "compiling version resource for PE metadata")
		}

		logger.Debug(ResCompile)
		b.FilesCreated = append(b.FilesCreated, ResObj)
		if b.Cmd(ResCompile) {
			CompileCommand += ResObj + " "
		} else {
			if !b.silent {
				b.SendConsoleMessage("Warning", "Failed to compile version resource - continuing without it")
			}
		}
	}
...
func (b *Builder) PatchConfig() ([]byte, error) {
	...
	if b.FileType == FILETYPE_WINDOWS_SERVICE_EXE {
		if val, ok := b.config.Config["Service Name"].(string); ok {
			if len(val) > 0 {
				...
				sanitizedVal := sanitizeServiceName(val)
				if sanitizedVal != val {
					if !b.silent {
						b.SendConsoleMessage("Warning", "Service name contained invalid characters and was sanitized")
					}
				}
				b.compilerOptions.Defines = append(b.compilerOptions.Defines, "SERVICE_NAME=\\\""+sanitizedVal+"\\\"")
				if !b.silent {
					b.SendConsoleMessage("Info", "set service name to "+sanitizedVal)
				}
			} else {
	...
	if val, ok := b.config.Config["Sleep Technique"].(string); ok && len(val) > 0 {
		...
		case "Legitimate":
			ConfigObfTechnique = SLEEPOBF_LEGITIMATE
			if !b.silent {
				b.SendConsoleMessage("Info", "sleep obfuscation \"Legitimate\" has been specified")
			}
			break
	...
	case handlers.LISTENER_HTTP:
		...
		psk := []byte(strings.TrimSpace(Config.Config.PSK))
		if len(psk) == 0 {
			generated := generateRandomPSK(16)
			Config.Config.PSK = generated
			psk = []byte(generated)
			if !b.silent {
				b.SendConsoleMessage("Warning", "HTTP listener had no PSK — auto-generated one for KEX")
			}
			logger.Warn("Auto-generated PSK for HTTP listener: " + Config.Config.Name)
		}
		DemonConfig.AddBytes(psk)
	...
	case handlers.LISTENER_PIVOT_SMB:
		...
		if len(psk) == 0 {
			...
			psk = []byte(generated)
			if !b.silent {
				b.SendConsoleMessage("Warning", "SMB listener had no PSK — auto-generated one for KEX")
			}
			logger.Warn("Auto-generated PSK for SMB listener: " + Config.Config.Name)
		}
		DemonConfig.AddBytes(psk)
...
func (b *Builder) Cmd(cmd string) bool {
	...
	// REJECT commands with dangerous patterns to prevent RCE
	dangerousPatterns := []string{";", "&&", "||", "|", "`", "$(", "${", "\n", "\r"}
	for _, pattern := range dangerousPatterns {
		if strings.Contains(cmd, pattern) {
			logger.Error("Security: dangerous pattern detected in compilation command: " + pattern)
			logger.Error("Security: aborting compilation to prevent command injection")
			if !b.silent {
				b.SendConsoleMessage("Error", "Security: dangerous pattern detected, compilation aborted")
				b.SendConsoleMessage("Error", "Blocked pattern: " + pattern)
			}
			// ABORT IMMEDIATELY - DO NOT EXECUTE
			return false
		}
	}
