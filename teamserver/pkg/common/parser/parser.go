...
// TrimMACAndVerify trims off the last 32 bytes as HMAC-SHA256 and verifies over the preceding data.
// Returns false on verification failure.
func (p *Parser) TrimMACAndVerify(AESMac []byte) bool {
	if len(p.buffer) < 32 || len(AESMac) < 32 {
		return false
	}
	dataLen := len(p.buffer) - 32
	mac := p.buffer[dataLen:]

	m := hmac.New(sha256.New, AESMac)
	m.Write(p.buffer[:dataLen])
	expected := m.Sum(nil)

	if !hmac.Equal(mac, expected) {
		return false
	}
	p.buffer = p.buffer[:dataLen]
	return true
}
