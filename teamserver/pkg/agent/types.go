...
type TeamServer interface {
	...
	AgentLoadFromDB(AgentID int) *Agent
...
// AuthorizedSocketOp tracks socket operations initiated by legitimate tasks
type AuthorizedSocketOp struct {
	TaskID      uint32
	SocketID    int32
	OpType      string // "rportfwd", "socks", etc.
	Authorized  bool
	CreatedAt   time.Time
}
...
type Agent struct {
	...
	// Track authorized socket operations to prevent SSRF exploitation
	AuthorizedSocketOps    []*AuthorizedSocketOp
	AuthorizedSocketOpsMtx sync.Mutex
	...
	// Dynamic command mapping per session
	DynamicCommands *DynamicCommandMap

	// Track the current operator for this agent session
...
// Dynamic command mapping per agent session
// ONLY includes the 22 main commands that flow through agent's DemonCommands[] dispatcher
type DynamicCommandMap struct {
	// Main Commands (22 total) - These flow through agent's command dispatcher table
	Sleep                     uint32  // DEMON_COMMAND_SLEEP = 11
	Checkin                   uint32  // DEMON_COMMAND_CHECKIN = 100  
	Job                       uint32  // DEMON_COMMAND_JOB = 21
	Proc                      uint32  // DEMON_COMMAND_PROC = 0x1010 (unused but in table)
	ProcList                  uint32  // DEMON_COMMAND_PROC_LIST = 12
	FS                        uint32  // DEMON_COMMAND_FS = 15
	InlineExecute             uint32  // DEMON_COMMAND_INLINE_EXECUTE = 20
	AssemblyInlineExecute     uint32  // DEMON_COMMAND_ASSEMBLY_INLINE_EXECUTE = 0x2001
	AssemblyVersions          uint32  // DEMON_COMMAND_ASSEMBLY_VERSIONS = 0x2003
	Config                    uint32  // DEMON_COMMAND_CONFIG = 2500
	Screenshot                uint32  // DEMON_COMMAND_SCREENSHOT = 2510
	Pivot                     uint32  // DEMON_COMMAND_PIVOT = 2520
	Net                       uint32  // DEMON_COMMAND_NET = 2100
	InjectDLL                 uint32  // DEMON_COMMAND_INJECT_DLL = 22
	InjectShellcode           uint32  // DEMON_COMMAND_INJECT_SHELLCODE = 24
	SpawnDLL                  uint32  // DEMON_COMMAND_SPAWN_DLL = 26
	Token                     uint32  // DEMON_COMMAND_TOKEN = 40
	Transfer                  uint32  // DEMON_COMMAND_TRANSFER = 2530
	Socket                    uint32  // DEMON_COMMAND_SOCKET = 2540
	Kerberos                  uint32  // DEMON_COMMAND_KERBEROS = 2550
	MemFile                   uint32  // DEMON_COMMAND_MEM_FILE = 2560
	Exit                      uint32  // DEMON_EXIT = 92
}
...
type AgentInfo struct {
	...
	PivotParentID uint32
	PivotHopCount uint32
