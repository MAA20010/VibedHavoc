...
func DatabaseNew(dbpath string) (*DB, error) {
	...
	if !db.existed {
		...
		/* Migrate existing DB to add any missing columns */
		db.Migrate()
	}
...
// Migrate adds missing columns to existing databases for session key persistence
func (db *DB) Migrate() error {
	// Add AESMac column if missing
	_, err := db.db.Exec(`ALTER TABLE TS_Agents ADD COLUMN "AESMac" string DEFAULT ''`)
	if err != nil {
		// Ignore error if column already exists (SQLite doesn't have IF NOT EXISTS for ALTER)
	}
	
	// Add PSK column if missing
	_, err = db.db.Exec(`ALTER TABLE TS_Agents ADD COLUMN "PSK" string DEFAULT ''`)
	if err != nil {
