...
func (s *Service) dispatch(response map[string]map[string]any, client *ClientService) {
	...
	case HeadAgent:
		...
		case BodyAgentRegister:
			...
			AgentInstance.MagicValue = uint32(Header.MagicValue)
			
			// Magic Value Debug for Service Registration
			logger.Debug(fmt.Sprintf("Service Registration - MagicValue: 0x%08x", Header.MagicValue))
			if agent.IsValidDemonMagic(uint32(Header.MagicValue)) {
				logger.Debug(fmt.Sprintf("Service magic validation PASSED for 0x%08x", Header.MagicValue))
			} else {
				logger.Debug(fmt.Sprintf("Service magic validation FAILED for 0x%08x", Header.MagicValue))
			}
