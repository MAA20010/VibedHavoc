...
PyObject* PythonAPI::Havoc::Core::Load( PyObject *self, PyObject *args )
{
    ...
    auto compiled = Py_CompileString( script.toStdString().c_str(), FilePath, Py_file_input );
    if ( compiled != NULL ) {
        auto mainMod = PyImport_AddModule( "__main__" );
        auto mainDict = PyModule_GetDict( mainMod );
        auto result = PyEval_EvalCode( compiled, mainDict, mainDict );
        Py_DECREF( compiled );
        if ( result != NULL ) {
            Py_DECREF( result );
        } else {
        ...
        spdlog::error( "Failed to compile script: {}", FilePath );
        Py_RETURN_FALSE;
    }
