...
GraphWidget::GraphWidget( QWidget* parent ) : QGraphicsView( parent )
{
    ...
    GraphScene->setSceneRect( -500, -500, 1000, 1000 );
    ...
    // Enable custom mouse panning behavior
...
Node* GraphWidget::GraphNodeAdd( SessionItem Session )
{
    ...
    // Update scene rect to accommodate new node
    updateDynamicSceneRect();
...
void GraphWidget::GraphNodeRemove( SessionItem Session )
{
    for ( int i = 0; i < NodeList.size(); i++ )
    {
        ...
        {
            // Save pointer before erasing (fixes crash)
            auto nodeToRemove = NodeList[ i ];
            
            GraphScene->removeItem( nodeToRemove->Node->NodeEdge );
            GraphScene->removeItem( nodeToRemove->Node );

            // Remove from parent BEFORE erasing from vector
            MainNode->Node->removeChild( nodeToRemove->Node );
            
            // Now safe to erase from vector
            NodeList.erase( NodeList.begin() + i );

            /* delete nodeToRemove->Node->NodeEdge;
            delete nodeToRemove->Node;
            delete nodeToRemove; */

            // Update scene rect after node removal
            updateDynamicSceneRect();
...
{
    // Use dynamic scene rect instead of fixed size
    updateDynamicSceneRect();
...
void GraphWidget::mousePressEvent( QMouseEvent* event )
{
    ...
    {
        // Check if we clicked on empty space (no item under mouse)
    ...
    // Let the base class handle node selection/interaction
...
void GraphWidget::scaleView( qreal scaleFactor )
{
    ...
    // Dynamic scene rect based on zoom level
    updateDynamicSceneRect();
}
...
void GraphWidget::updateDynamicSceneRect()
{
    ...
    dynamicSize = qMax( dynamicSize, qreal(1000) );
    ...
    GraphScene->setSceneRect( newSceneRect );
}
...
void Node::calculateForces()
{
    ...
    // Allow unlimited node movement - no scene boundary constraints
