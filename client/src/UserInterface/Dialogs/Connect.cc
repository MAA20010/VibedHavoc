...
Util::ConnectionInfo HavocNamespace::UserInterface::Dialogs::Connect::StartDialog( bool FromAction )
{
    ...
    if ( this->tryConnect )
    {
        ...
        if ( this->isNewProfile ) {
            if ( this->dbManager && ! this->dbManager->addTeamserverInfo( *ConnectionInfo ) ) {
                spdlog::warn( "Failed to add Teamserver Info to database" );
            } else if ( !this->dbManager ) {
                spdlog::warn( "DBManager unavailable - cannot save teamserver profile to database" );
            }
        }
...
void HavocNamespace::UserInterface::Dialogs::Connect::onButton_Connect()
{
    ...
    if ( this->dbManager && this->dbManager->checkTeamserverExists( lineEdit_Name->text() ) && this->isNewProfile )
    {
        auto MessageBox = QMessageBox();
