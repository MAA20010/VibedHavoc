...
void Payload::setupUi( QDialog* Dialog )
{
    ...
    connect( ComboListener, &QComboBox::currentTextChanged, this, [&]( const QString& text ){

        // refresh config when listener changes — SMB hides sleep/jitter
        if ( ComboAgentType->currentText().compare( "Demon" ) == 0 )
            DefaultConfig();
...
auto Payload::DefaultConfig() -> void
{
    ...
    /* Check if selected listener is SMB — sleep/jitter are irrelevant for SMB agents
     * because they use event-driven pipe waits with hardcoded timeouts + jitter. */
    auto IsSMB = false;
    auto SelectedListener = ComboListener->currentText().toStdString();
    for ( auto& Listener : HavocX::Teamserver.Listeners )
    {
        if ( Listener.Name == SelectedListener && Listener.Protocol == HavocSpace::Listener::PayloadSMB.toStdString() )
        {
            IsSMB = true;
            break;
        }
    ...
    /* SMB agents use event-driven pipe waits — sleep is just the housekeeping
     * timeout (commands are always instant).  Default to 420s (7min) / 50% jitter.
     * HTTP agents use the profile defaults. Operator can override either. */
    auto ConfigSleepLineEdit  = new QLineEdit( QString::number( IsSMB ? 420 : DemonConfig[ "Sleep" ].toInt() ) );
    auto ConfigJitterLineEdit = new QLineEdit( QString::number( IsSMB ? 50  : Jitter ) );
    ...
    SleepObfTechnique->addItems( QStringList() << "WaitForSingleObjectEx" << "Foliage" << "Ekko" << "Zilean" << "Legitimate" );
    ...
    else if ( DefaultSleepTechnique == "Legitimate" )
        SleepObfTechnique->setCurrentIndex( 4 );
