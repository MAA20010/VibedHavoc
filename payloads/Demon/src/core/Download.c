...
PDOWNLOAD_DATA DownloadAdd( HANDLE hFile, LONGLONG MaxSize, BOOL Aggressive )
{
    ...
    Download->Aggressive = Aggressive;
    ...
    PRINTF( "Instance->Downloads => %p (Aggressive: %s)\n", Instance->Downloads, Aggressive ? "TRUE" : "FALSE" )
...
VOID DownloadPush()
{
    ...
    for ( ;; )
    {
        ...
        if ( Download->State == DOWNLOAD_STATE_RUNNING )
        {
            ...
            PRINTF( "Download (%x) is in state DOWNLOAD_STATE_RUNNING (Aggressive: %s)\n", Download->FileID, Download->Aggressive ? "TRUE" : "FALSE" )

            /* Aggressive mode sends all chunks in one loop, bypassing sleep */
            do {
                /* Reset memory. */
                MemSet( Instance->DownloadChunk.Buffer, 0, Instance->DownloadChunk.Length );
                ...
                /* Break after first chunk if not aggressive mode */
            } while ( Download->Aggressive && Download->Size > 0 && Read > 0 );
        }
...
PMEM_FILE NewMemFile( ULONG32 ID, SIZE_T Size, PVOID Data, ULONG32 ReadSize )
{
    ...
    PRINTF( "Creating new MemFile ID=%x, Size=%lx bytes\n", ID, Size );

    MemFile           = MmHeapAlloc( sizeof( MEM_FILE ) );
    if ( !MemFile )
    {
        PUTS( "Failed to allocate MEM_FILE structure" );
        return NULL;
    }
    ...
    PRINTF( "Attempting to allocate %lx bytes for MemFile data\n", Size );
    MemFile->Data     = MmHeapAlloc( MemFile->Size );

    if ( ! MemFile->Data )
    {
        PRINTF( "Failed to allocate %lx bytes for MemFile data\n", MemFile->Size );
        MmHeapFree( MemFile );
        return NULL;
    }

    PRINTF( "Successfully allocated MemFile data, copying %x bytes\n", ReadSize );
...
BOOL RemoveMemFile( ULONG32 ID )
{
    ...
    Last    = NULL;  // Start with NULL to handle first item correctly
    ...
    for ( ;; )
    {
        ...
        if ( MemFile->ID == ID )
        {
            ...
            if ( Last )
            {
                // Not the first item - update previous item's Next pointer
                Last->Next = MemFile->Next;
            }
            else
            {
                // First item in list - update Instance->MemFiles
...
// Clean up all MemFiles to prevent resource conflicts
VOID CleanupAllMemFiles( VOID )
{
    PMEM_FILE MemFile = Instance->MemFiles;
    PMEM_FILE NextFile = NULL;
    UINT32 Count = 0;
    
    PUTS( "Cleaning up all existing MemFiles" );
    
    while ( MemFile )
    {
        NextFile = MemFile->Next;
        PRINTF( "Cleaning up MemFile [%x]\n", MemFile->ID );
        MemFileFree( MemFile );
        Count++;
        MemFile = NextFile;
    }
