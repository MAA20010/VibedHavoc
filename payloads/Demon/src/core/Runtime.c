...
BOOL RtUser32(
    ...
    if ( ( Instance->Modules.User32 = LdrModuleLoad( ModuleName ) ) ) {
        ...
        // Add missing User32 APIs for InjectAdvanced functionality
        Instance->Win32.GetWindowThreadProcessId    = LdrFunctionAddr( Instance->Modules.User32, H_FUNC_GETWINDOWTHREADPROCESSID );
        Instance->Win32.IsWindowVisible             = LdrFunctionAddr( Instance->Modules.User32, H_FUNC_ISWINDOWVISIBLE );
        Instance->Win32.EnumWindows                 = LdrFunctionAddr( Instance->Modules.User32, H_FUNC_ENUMWINDOWS );
        Instance->Win32.PostMessageA                = LdrFunctionAddr( Instance->Modules.User32, H_FUNC_POSTMESSAGEA );
        Instance->Win32.SetTimer                    = LdrFunctionAddr( Instance->Modules.User32, H_FUNC_SETTIMER );
        Instance->Win32.KillTimer                   = LdrFunctionAddr( Instance->Modules.User32, H_FUNC_KILLTIMER );
