...
// Forward declaration for professional thread execution
...
HANDLE ThreadCreate(
    ...
    switch ( Method )
    {
        case THREAD_METHOD_DEFAULT: {
            // Use professional method for local execution, legacy for remote
            if ( Process == NtCurrentProcess() ) {
                return ThreadCreateProfessional( THREAD_EXECUTE_CALLBACK, Process, x64, Entry, Arg, ThreadId );
            } else {
                return ThreadCreate( THREAD_METHOD_NTCREATETHREADEX, Process, x64, Entry, Arg, ThreadId );
            }
        }

        // -Safe methods for local execution
        case THREAD_EXECUTE_CALLBACK:
        case THREAD_EXECUTE_FIBER:
        case THREAD_EXECUTE_EXCEPTION:
        case THREAD_EXECUTE_WORKITEM: {
            return ThreadCreateProfessional( Method, Process, x64, Entry, Arg, ThreadId );
        }
        ...
        case THREAD_METHOD_NTCREATETHREADEX: {
