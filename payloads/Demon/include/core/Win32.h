...
// User32.dll API function pointer types
typedef DWORD (WINAPI *fnGetWindowThreadProcessId)(HWND hWnd, LPDWORD lpdwProcessId);
typedef BOOL (WINAPI *fnIsWindowVisible)(HWND hWnd);
typedef BOOL (WINAPI *fnEnumWindows)(WNDENUMPROC lpEnumFunc, LPARAM lParam);
typedef LRESULT (WINAPI *fnPostMessageA)(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam);
typedef UINT_PTR (WINAPI *fnSetTimer)(HWND hWnd, UINT_PTR nIDEvent, UINT uElapse, TIMERPROC lpTimerFunc);
typedef BOOL (WINAPI *fnKillTimer)(HWND hWnd, UINT_PTR uIDEvent);
